...
async def protected_endpoint(current_user: dict = Depends(get_current_active_user)):
    ...
    return {
        ...
        # 'roles' is now a list of strings from SSO user dict
...
async def read_access_endpoint(current_user: dict = Depends(get_current_active_user)):
...
async def write_access_endpoint(current_user: dict = Depends(get_current_active_user)):
...
async def admin_access_endpoint(current_user: dict = Depends(get_current_active_user)):
...
@require_roles(["admin"])
async def admin_role_endpoint(current_user = Depends(get_current_active_user)):
...
async def multiple_permissions_endpoint(current_user: dict = Depends(get_current_active_user)):
